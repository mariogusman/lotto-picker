...
# List of numbers for the Mega-Sena along with the probability/weight for drawing them
...
# List of numbers for the Loto-facil along with the probability/weight for drawing them
...
def drawMegaNumbersUnique():
    ...
    i = 0
    print("Mega-Sena:\n")
    #if megaDrawnNumbers has less than 6 numbers, draw more numbers
    while len(megaDrawnNumbers) < 6:
        # Generate a random number between 1 and 25 using the probability distribution of the numbers
        megaGenerated = np.random.choice(megaNumbers, p=megaProbability)
        # Add the number to the list of drawn numbers if not already in the list
        if megaGenerated not in megaDrawnNumbers:
            megaDrawnNumbers.append(megaGenerated)
            print("Number " + str(i+1) + ": " + str(megaGenerated) + " (" + str(round(megaProbability[megaGenerated-1]*100, 2)) + "%)")
...
def drawLotoNumbersUnique():
    ...
    i = 0
    print("Loto Fácil:\n")
    #if lotoDrawnNumbers has less than 6 numbers, draw more numbers ea
    while len(lotoDrawnNumbers) < 15:
        # Generate a random number between 1 and 25 using the probability distribution of the numbers
        lotoGenerated = np.random.choice(lotoNumbers, p=lotoProbability)
        # Add the number to the list of drawn numbers if not already in the list
        if lotoGenerated not in lotoDrawnNumbers:
            lotoDrawnNumbers.append(lotoGenerated)
            print("Number " + str(i+1) + ": " + str(lotoGenerated) + " (" + str(round(lotoProbability[lotoGenerated-1]*100, 2)) + "%)")
